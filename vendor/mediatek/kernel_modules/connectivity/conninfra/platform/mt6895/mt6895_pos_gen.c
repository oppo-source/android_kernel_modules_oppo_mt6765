...
static int connsys_a_die_efuse_read_adie6637_check_efuse_valid(bool *pefuse_valid, unsigned int efuse_addr) {
	#ifndef CONFIG_FPGA_EARLY_PORTING
...
{
	return connsys_a_die_efuse_read_adie6637_check_efuse_valid(pefuse_valid, 0);
}
...
int connsys_a_die_efuse_read_mt6895_gen(unsigned int efuse_addr) {
	int ret[4];
	unsigned int efuse_list[4] = { 0 };
	bool efuse_valid;

	ret[0] = connsys_a_die_efuse_read_adie6637_check_efuse_valid(&efuse_valid, efuse_addr);
	if (ret[0] < 0 || efuse_valid == false) {
		pr_notice("%s failed. ret = %d, efuse_valid = %d\n", __func__, ret[0], efuse_valid);
		return -1;
	}
	...
	pr_info("%s addr = 0x%x efuse = [0x%08x, 0x%08x, 0x%08x, 0x%08x]", __func__, efuse_addr, efuse_list[0], efuse_list[1], efuse_list[2], efuse_list[3]);
...
int connsys_a_die_cfg_adie6637_PART2_mt6895_gen(unsigned int hw_ver_id)
{
	...
	#ifndef CONFIG_FPGA_EARLY_PORTING
		consys_spi_read_nolock_mt6895(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_RG_BT0_BG_TIME, &val);
		pr_info("%s before write 0x20003 to RG_BT0_BG_TIME = 0x%08x\n", __func__, val);
		consys_spi_write_nolock_mt6895(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_RG_BT0_BG_TIME, 0x20003);
		consys_spi_read_nolock_mt6895(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_RG_BT0_BG_TIME, &val);
		pr_info("%s after write 0x20003 to RG_BT0_BG_TIME = 0x%08x\n", __func__, val);
	#endif
	...
	#ifndef CONFIG_FPGA_EARLY_PORTING
		consys_spi_read_nolock_mt6895(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_RG_TOP_XO_02, &val);
		pr_info("%s before write 0x88000000 to ATOP_RG_TOP_XO_02 = 0x%08x\n", __func__, val);
		consys_spi_write_nolock_mt6895(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_RG_TOP_XO_02, 0x88000000);
		consys_spi_read_nolock_mt6895(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_RG_TOP_XO_02, &val);
		pr_info("%s after write 0x88000000 to ATOP_RG_TOP_XO_02 = 0x%08x\n", __func__, val);
	#endif
