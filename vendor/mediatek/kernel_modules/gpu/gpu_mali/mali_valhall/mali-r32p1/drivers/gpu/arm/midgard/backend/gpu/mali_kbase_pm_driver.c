...
static bool core_mask_update_done(struct kbase_device *kbdev)
{
	...
	spin_lock_irqsave(&kbdev->hwaccess_lock, flags);
	if (!kbase_pm_is_mcu_desired(kbdev) || !kbdev->pm.backend.gpu_ready ||
	    (kbdev->pm.backend.mcu_state == KBASE_MCU_ON))
		update_done = true;
	spin_unlock_irqrestore(&kbdev->hwaccess_lock, flags);
...
{
	unsigned long timeout = kbase_csf_timeout_in_jiffies(PM_TIMEOUT_MS);
	long remaining;
	int err = 0;
...
static void update_user_reg_page_mapping(struct kbase_device *kbdev)
{
	...
	list_for_each_entry (kctx, &kbdev->csf.user_reg.list, csf.user_reg.link) {
		if ((kbdev->pm.backend.gpu_powered && kctx->csf.user_reg.dummy_page_mapped) ||
		    (!kbdev->pm.backend.gpu_powered && kctx->csf.user_reg.real_page_mapped)) {
			/* This would zap the pte corresponding to the mapping of User
			 * register page of the Kbase context.
			 */
			unmap_mapping_range(kctx->csf.user_reg.filp->f_inode->i_mapping,
			...
			dev_dbg(kbdev->dev, "Updated user reg page mapping of ctx %d_%d",
				kctx->tgid, kctx->id);
		}
	}
