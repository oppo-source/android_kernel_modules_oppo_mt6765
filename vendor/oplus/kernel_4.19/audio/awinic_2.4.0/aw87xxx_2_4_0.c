...
#ifdef CONFIG_SND_SOC_OPLUS_PA_MANAGER
void *oplus_pa_aw_2_4_0_node = NULL;
uint8_t aw_pa_mode_2_4_0 = AW87XXX_MUSIC_MODE;
#endif /* CONFIG_SND_SOC_OPLUS_PA_MANAGER */

/*****************************************************************
* aw87xxx marco
******************************************************************/
#define AW87XXX_I2C_NAME	"aw87xxx_pa_2_4_0"
...
static char *aw_profile[] = {"Music", "Off"};/*aw87xxx_acf.bin audio sence config*/
...
int aw87xxx_update_profile(struct aw87xxx *aw87xxx, char *profile)
{
	...
	aw87xxx_2_4_0_monitor_stop(&aw87xxx->monitor);
	if (0 == strncmp(profile, aw87xxx->prof_off_name, AW_PROFILE_STR_MAX)) {
		ret = aw87xxx_power_down(aw87xxx, profile);
	} else {
		ret = aw87xxx_power_on(aw87xxx, profile);
		if (!ret)
			aw87xxx_2_4_0_monitor_start(&aw87xxx->monitor);
	}
...
#ifdef CONFIG_SND_SOC_OPLUS_PA_MANAGER
int aw87xxx_2_4_0_spk_low_voltage_status = 0;
...
static void aw87xxx_audio_spk_low_voltage_status(int bStatus)
{
	...
	aw87xxx_2_4_0_spk_low_voltage_status = bStatus;
	...
	list_for_each(pos, &g_aw87xxx_list) {
		...
		if (aw87xxx_2_4_0_spk_low_voltage_status > 0) {
...
void aw87xxx_2_4_0_enable_pa(int enable, int mode, int32_t channel) {
	pr_info("%s: enable = %d, mode = %d, channel = %d\n", __func__, enable, mode, channel);

	//int a  = AW_CTOS_PROFILE_MUSIC;
...
void aw87xxx_2_4_0_enable_l_pa(int enable, int mode){
	if(enable == 1){
		/* set PA AW_DEV_0 to Music sence */
		aw87xxx_set_profile(AW_DEV_0, aw_profile[0]);
	}else{
		/* set PA AW_DEV_0 to Off sence */
		aw87xxx_set_profile(AW_DEV_0, aw_profile[1]);
	}
	pr_info("%s,enable pa\n", __func__);
}

void aw87xxx_2_4_0_enable_r_pa(int enable, int mode){
	//aw87xxx_enable_pa(enable, mode, AW87XXX_RIGHT_CHANNEL);
	pr_info("%s\n", __func__);
}

int ext_2_4_0_amp_low_voltage_set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol){
...
int ext_2_4_0_amp_low_voltage_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
        ucontrol->value.integer.value[0] = aw87xxx_2_4_0_spk_low_voltage_status;
        pr_info("%s: aw87xxx_2_4_0_spk_low_voltage_status = %d\n", __func__, aw87xxx_2_4_0_spk_low_voltage_status);
		pr_info("%s\n", __func__);
        return 0;
}

int aw87xxx_2_4_0_spk_mode_set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol){
...
int aw87xxx_2_4_0_spk_mode_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol){
	/* pr_info("%s() = %d\n", __func__, aw_pa_mode);
...
bool is_mute_2_4_0_status = false;
int aw87xxx_2_4_0_speaker_mute_set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct soc_enum *e = (struct soc_enum *)kcontrol->private_value;
	...
	if(ucontrol->value.integer.value[0]){
		...
		aw87xxx_set_profile(AW_DEV_0, aw_profile[1]);
		is_mute_2_4_0_status = true;
	}else{
		//set pa to Music sence:ummute
		aw87xxx_set_profile(AW_DEV_0, aw_profile[0]);
		is_mute_2_4_0_status = false;
	}

	return 0;
}

int aw87xxx_2_4_0_speaker_mute_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	pr_info("%s() = %d\n", __func__, is_mute_2_4_0_status);
	ucontrol->value.integer.value[0] = is_mute_2_4_0_status;
	return 0;
}
#endif /*CONFIG_SND_SOC_OPLUS_PA_MANAGER*/
				...
				const struct i2c_device_id *id)
{
	...
	/*monitor init*/
	aw87xxx_2_4_0_monitor_init(aw87xxx->dev, &aw87xxx->monitor, dev_node);
	...
	speaker_device = get_speaker_dev( 1 );
	...
	if (speaker_device !=NULL ) {
		...
		speaker_device->speaker_enable_set = aw87xxx_2_4_0_enable_l_pa;
		speaker_device->boost_voltage_set = ext_2_4_0_amp_low_voltage_set;
		speaker_device->boost_voltage_get = ext_2_4_0_amp_low_voltage_get;
		speaker_device->spk_mode_set = aw87xxx_2_4_0_spk_mode_set;
		speaker_device->spk_mode_get = aw87xxx_2_4_0_spk_mode_get;
		speaker_device->speaker_mute_set = aw87xxx_2_4_0_speaker_mute_set;
		speaker_device->speaker_mute_get = aw87xxx_2_4_0_speaker_mute_get;
	...
	if (new_speaker_device_node == true) {
		...
		oplus_pa_aw_2_4_0_node = oplus_speaker_pa_register(speaker_device);
	}
...
static const struct of_device_id extpa_of_match[] = {
	{.compatible = "awinic,aw87xxx_pa_2_4_0"},
	{},
};
