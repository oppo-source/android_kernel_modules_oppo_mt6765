...
unsigned int soc_sipa81xx_get_aux_num(
...
EXPORT_SYMBOL(soc_sipa81xx_get_aux_num);

unsigned int soc_sipa81xx_get_codec_conf_num(
	struct platform_device *pdev)
{
	return soc_sipa81xx_get_aux_num(pdev);
}
EXPORT_SYMBOL(soc_sipa81xx_get_codec_conf_num);

int soc_sipa81xx_init(
	struct platform_device *pdev,
	...
	u32 conf_num)
{
	...
	dev_num = soc_sipa81xx_get_aux_num(pdev);
...
EXPORT_SYMBOL(soc_sipa81xx_init);

int soc_sipa81xx_deinit(
	struct platform_device *pdev,
	struct snd_soc_aux_dev *aux_dev,
	u32 aux_num,
	struct snd_soc_codec_conf *codec_conf,
...
EXPORT_SYMBOL(soc_sipa81xx_deinit);
	...
	struct snd_soc_card *card)
{
	...
	aux_num = soc_sipa81xx_get_aux_num(pdev);
	...
	ret = soc_sipa81xx_init(pdev, aux_dev, aux_num, codec_conf, conf_num);
	if (0 != ret) {
		pr_err("[  err][%s] : soc_sipa81xx_init ret = %d !!! \r\n",
			__func__, ret);
		return ret;
	}
...
EXPORT_SYMBOL(soc_aux_init_only_sipa81xx);

int soc_aux_deinit_only_sipa81xx(
	struct platform_device *pdev,
	struct snd_soc_card *card)
{
	if (NULL == pdev) {
