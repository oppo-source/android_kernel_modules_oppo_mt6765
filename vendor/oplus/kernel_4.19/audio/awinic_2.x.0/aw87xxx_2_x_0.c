...
int ext_2_x_0_amp_low_voltage_set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol){
	if (ucontrol->value.integer.value[0] >= 2) {
		aw87xxx_set_boost_voltage( DEV_INDEX_0 ,  VOL_STATUS_HIGH );
		aw87xxx_set_boost_voltage( DEV_INDEX_1 ,  VOL_STATUS_HIGH );
		//aw87xxx_audio_spk_low_voltage_status(0);
	} else {
		aw87xxx_set_boost_voltage( DEV_INDEX_0 ,  VOL_STATUS_LOW );
		aw87xxx_set_boost_voltage( DEV_INDEX_1 ,  VOL_STATUS_LOW );
