...
extern int cts_register_vendor_dirver(struct chipone_ts_data *cts_data);
...
#endif
{
...
#endif
#endif
	ret = cts_register_vendor_dirver(cts_data);
	if (ret < 0) {
		cts_err("register vendor driver failed");
		goto err_register;
	}
    ...
    if (ret) {
        cts_err("Init vendor specific failed %d", ret);
        goto err_vendor_deinit;
    }
#endif
