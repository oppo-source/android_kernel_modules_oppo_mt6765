...
unsigned int g_rwsem_max_block_time_ms = 5000;
...
static bool waiter_block_too_long(struct rwsem_waiter *waiter)
{
	...
	if (block_start) {
		...
		if ((s64)delta < 0)
			delta = 0;
		delta >>= 20;
		if (delta > g_rwsem_max_block_time_ms)
			goto stop_boost;
	}
...
module_param_named(rwsem_max_block_time_ms, g_rwsem_max_block_time_ms, uint, 0660);
