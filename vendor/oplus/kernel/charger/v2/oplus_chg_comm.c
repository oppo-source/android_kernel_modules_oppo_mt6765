...
#define OPLUS_NOTIFY_S0C_JUMP_THD			3
static void oplus_comm_battery_notify_soc_check(struct oplus_chg_comm *chip)
{
	...
	soc = chip->soc;
	uisoc = chip->ui_soc;
	if (!pre_soc && !pre_uisoc) {
		pre_soc = chip->soc;
		pre_uisoc = chip->ui_soc;
	}

	if (soc == pre_soc && uisoc == pre_uisoc && uisoc == pre_soc) {
		chip->gauge_soc_jump = false;
	} else if (abs(pre_soc - soc) > OPLUS_NOTIFY_S0C_JUMP_THD) {
		chip->gauge_soc_jump = true;
		chg_err("The gap between soc and pre_soc is too large\n");
	} else if (abs(pre_uisoc - uisoc) > OPLUS_NOTIFY_S0C_JUMP_THD) {
		chip->gauge_soc_jump = true;
		chg_err("The gap between uisoc and pre_uisoc is too large\n");
	} else if (abs(uisoc - pre_soc) > OPLUS_NOTIFY_S0C_JUMP_THD) {
		chip->gauge_soc_jump = true;
		chg_err("The gap between pre_soc and uisoc is too large\n");
	}
...
static int oplus_comm_tbatt_power_off_kthread(void *arg)
{
	...
	while (!kthread_should_stop()) {
		...
		chip->batt_temp = data.intval + chip->spec.tbatt_power_off_cali_temp;
		if (chip->batt_temp > pwroff_emerg_th) {
			emergency_count++;
			chg_err(" emergency_count:%d \n", emergency_count);
		} else {
			emergency_count = 0;
		}
		if (chip->batt_temp > pwroff_over_temp_th) {
			over_temp_count++;
