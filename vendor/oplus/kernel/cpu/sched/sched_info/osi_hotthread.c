...
void jank_hotthread_update_tick(struct task_struct *p, u64 now)
{
	...
	timestamp_prewin = hot_track[now_idx][0].timestamp;
	if (!is_same_idx(timestamp_prewin, now)) {
		memset(&hot_track[now_idx], 0, TOP_THREAD_CNT * sizeof(struct task_track));
		/*Note :do_each_thread can not iter swapper thread*/
		for_each_possible_cpu(iter_cpu) {
			task = per_cpu(runqueues, iter_cpu).idle;
			ots = get_oplus_task_struct(task);
			insert_hot_thread(ots, task, task->group_leader, now_idx);
			reset_perthread_cnt(ots);
		}
		...
		get_hot_thread(now_idx);
		hot_track[now_idx][0].timestamp = now;
	}
