...
#define GYRO_CALI_RANGE             (0x07)
#define IS_SUPPORT_MTK_CALI         (0x08)
#define CALI_ADAPT_Q                (0x09)
#define GOLD_ALS_FACTOR             (0x0C)
...
static bool cali_adapt_q = false;
...
static void get_cali_nv_adapt_q_flag(void)
{
	void *fdt = initial_boot_params;
	int offset = 0;
	int is_support = 0;
...
static void get_accgyro_cali_version(void)
{
    ...
    DEVINFO_LOG("gyro_cali_range = %d", gyro_cali_range);
...
static int oplus_sensor_feature_init()
{
    ...
    p_entry = proc_create_data("cali_adapt_q", S_IRUGO, oplus_sensor, &Sensor_info_fops,
            UINT2Ptr(CALI_ADAPT_Q));
    if (!p_entry) {
        DEVINFO_LOG("cali_adapt_q err\n ");
        return -ENOMEM;
    }
