...
/*
*Interface for lcd to control gpio pin
*/
int tp_control_gpio_status(bool enable)
{
	int retry = 0;
	...
	if (gpio_is_valid(g_tp->hw_res.reset_gpio)) {
		gpio_set_value(g_tp->hw_res.reset_gpio, enable);
		TPD_INFO("set reset gpio[%d] %s, value is %d.\n",
			g_tp->hw_res.reset_gpio, enable ? "enable" : "disable",
			gpio_get_value(g_tp->hw_res.reset_gpio));
	}
...
EXPORT_SYMBOL(tp_control_gpio_status);
