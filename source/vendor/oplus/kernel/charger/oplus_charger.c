...
if (chip == NULL) {
	...
	switch (psp) {
		...
		case POWER_SUPPLY_PROP_VOLTAGE_MIN:
			...
			break;
		case POWER_SUPPLY_PROP_CURRENT_NOW:
			if (oplus_chg_get_voocphy_support() == false && oplus_vooc_get_fastchg_started() == true) {
		...
		case POWER_SUPPLY_PROP_CHARGE_NOW:
			...
			&& oplus_vooc_get_fast_chg_type() != CHARGER_SUBTYPE_FASTCHG_VOOC) {
				...
				if (oplus_chg_get_voocphy_support() == false) {
		...
		char __user *buff, size_t count, loff_t *off)
{
	...
	int len = 0;
	int asic_current = 0;
	int cp_ibus, slave_cp_ibus, cp_vbat_deviation;
...
int oplus_chg_parse_charger_dt(struct oplus_chg_chip *chip)
{
	...
	chip->voocphy_support =  of_property_read_bool(node, "qcom,voocphy_support");
...
static bool oplus_chg_check_tbatt_is_good(struct oplus_chg_chip *chip)
{
	...
	if (tbatt_status != chip->tbatt_status) {
		...
		vbatt_higherthan_4180mv = false;
		if ((!chip->voocphy_support && oplus_vooc_get_allow_reading() == true)
			|| (chip->voocphy_support && oplus_vooc_get_fastchg_started() == false)) {
			oplus_chg_set_float_voltage(chip);
			oplus_chg_set_charging_current(chip);
		}
...
//extern void oplus_vooc_reset_variables(void);
void oplus_chg_variables_reset(struct oplus_chg_chip *chip, bool in)
{
	if (in) {
		...
		if(!oplus_chg_show_vooc_logo_ornot() && !oplus_chg_get_voocphy_support()) {
...
static void oplus_chg_get_battery_data(struct oplus_chg_chip *chip)
{
	...
	if (oplus_vooc_get_fastchg_started() == true) {
		...
		} else if (oplus_chg_get_voocphy_support() && oplus_voocphy_get_external_gauge_support() == 0) {
			gauge_volt = oplus_gauge_get_prev_batt_mvolts();
...
static void oplus_chg_get_chargerid_voltage(struct oplus_chg_chip *chip)
{
	...
	if (oplus_chg_get_voocphy_support() == true && oplus_voocphy_stage_check() == false) {
...
static void oplus_chg_fast_switch_check(struct oplus_chg_chip *chip)
{
	...
	if(oplus_chg_get_voocphy_support() && chip->pd_svooc == false 
...
static void oplus_chg_pd_config(struct oplus_chg_chip *chip)
{
	...
	if (chip->dual_charger_support || oplus_chg_get_voocphy_support()) {
		if (chip->charger_volt > 7500) {
			chip->pd_chging = true;
		} else {
			chip->pd_chging = false;
	...
	if (chip->chg_ops->get_charger_subtype() == CHARGER_SUBTYPE_PD) {
		if ( (pdqc_9v == true)
			...
			if (ret >= 0) {
				...
				if (chip->charger_volt > 7500) {
					pdqc_9v = true;
					chg_err("set to 5V failed");
				} else {
					chg_err("set to 5V success");
					pdqc_9v = false;
				...
				/*set as false to avoid repeat configuration*/
				pdqc_9v = false;
			}
		}
...
static void oplus_chg_qc_config(struct oplus_chg_chip *chip)
{
	...
	chg_err("chip->charger_type[%d], subtype[%d], charger_volt[%d]\n",
		chip->charger_type, chip->chg_ops->get_charger_subtype(), chip->charger_volt);
	...
	if ((qc_chging == false)
		...
		if (ret >= 0) {
			...
			if (chip->charger_volt > 7500) {
				...
				qc_chging = false;
				chg_err("QC config failed");
			}
		} else {
			/*set as true to avoid repeat configuration*/
			qc_chging = true;
		}
	}
	return;
...
void oplus_chg_platform_gauge_choose_curve(void)
{
	...
	if (g_charger_chip->chg_ops->get_platform_gauge_curve)
		g_charger_chip->chg_ops->get_platform_gauge_curve();
}
...
bool oplus_chg_get_voocphy_support(void)
{
	if (!g_charger_chip) {
		return false;
	} else {
...
int oplus_convert_current_to_level(struct oplus_chg_chip *chip, int val)
{
	...
	if(reply_bits == 7) {
		if(is_vooc_support_single_batt_svooc() == 1 || oplus_chg_get_voocphy_support()) {
