...
struct oplus_voocphy_manager *g_voocphy_chip = NULL;
...
int oplus_voocphy_reset_variables(struct oplus_voocphy_manager *chip)
{
	...
	chip->batt_temp_plugin = BATT_TEMP_NORMAL;
	if (chip->fastchg_reactive == false) {
...
void oplus_voocphy_get_soc_and_temp_with_enter_fastchg(struct oplus_voocphy_manager *chip)
{
	...
	if (batt_temp < chip->vooc_little_cold_temp) { /*0-5C*/
		...
		chip->batt_temp_plugin = BATT_TEMP_LITTLE_COLD;
		...
		chip->batt_temp_plugin = BATT_TEMP_COOL;
		...
		chip->batt_temp_plugin = BATT_TEMP_LITTLE_COOL;
...
{
	struct oplus_chg_chip *g_charger_chip = oplus_chg_get_chip();
...
int oplus_voocphy_vol_event_handle(unsigned long data)
{
	...
	if (!chip->btb_temp_over) {
		...
		if ((chip->batt_temp_plugin == BATT_TEMP_LITTLE_COLD			//0-5 chg to 4430mV
		     && chip->gauge_vbatt > TEMP0_2_TEMP45_FULL_VOLTAGE)
		    || (chip->batt_temp_plugin == BATT_TEMP_COOL				//5-12 chg to 4430mV
...
int oplus_voocphy_commu_process_handle(unsigned long enable)
{
	...
	//note exception for systrace
	//trace_oppo_tp_sched_change_ux(duration, task_cpu(current));
	//trace_oppo_tp_sched_change_ux(0, task_cpu(current));
