...
#if (!defined(CONFIG_OPLUS_CHARGER_MTK)) && (!defined(CONFIG_OPLUS_SM7250R_CHARGER))
...
void oplus_optiga_read_battery_model(void)
{
	int i;
	uint8_t NVM_READ_DATA[16] = {0};
	...
	if (Nvm_ReadData(8, 16, NVM_READ_DATA) == FALSE) {
		chg_err("cannot read NVM PAGE!\n");
	} else {
		for (i=0; i<16; i++)
			chg_err("wenboyu NVM_BUFF[%d] = 0x%0x\n", i, NVM_READ_DATA[i]);
	}
}
...
int oplus_optiga_get_external_auth_hmac(void) {
	...
	oplus_optiga_read_battery_model();
