...
struct iio_channel *chgntc_chan; //chgntc_temp_auxadc_channel
...
*/
	chgntc_chan = iio_channel_get(dev, "chargerntc-ch3");
	if(IS_ERR_OR_NULL(chgntc_chan)){
		chg_err("chgntc_chan init fial,err = %d",PTR_ERR(chgntc_chan));
		ret = -1;
	}
...
/*static int oplus_get_usbtemp_volt(void)
{
	int usbtemp_volt = 0;

	iio_read_channel_processed(batt_id, &usbtemp_volt);
	if (usbtemp_volt <= 0) {
...
#ifdef CONFIG_CHARGER_SY6970
	info->data.chgntc_pin = of_get_named_gpio(np, "chgntc_gpio", 0);
	if(info->data.chgntc_pin < 0)	{
		chg_err("[%s]: chgntc_pin < 0 !!! \r\n", __func__);
	}

	if (gpio_request(info->data.chgntc_pin, "CHGNTC_GPIO") < 0)
		chg_err("[%s]: chgntc_pin gpio_request fail !!! \r\n", __func__);

	if(usbtemp_channel_init(dev))
...
void oplus_wake_up_usbtemp_thread(void)
{
	if (oplus_usbtemp_check_is_support() == true) {
		if (is_usbtemp_thread_init_done == true){
			...
			pr_err("wake_up_usbtemp_thread, vbus:%d, otg:%d",oplus_chg_get_vbus_status(g_oplus_chip),oplus_chg_get_otg_online());
		}
	}
}
...
static void battery_set_qc_current(void)
{
	struct oplus_chg_chip *chip = g_oplus_chip;
	int charging_current = 3400;
	int current_limit = 2000;

	if (!chip || !chip->dual_charger_support) {
		return;
	}
	...
	if (CHARGER_SUBTYPE_QC == chip->chg_ops->get_charger_subtype()) {
		if (!chip->sub_chg_ops && !chip->sub_chg_ops->charging_enable) {
			chip->sub_chg_ops->charging_enable();
		}

		current_limit = chip->limits.qc_input_current_charger_ma;
		charging_current = chip->limits.temp_normal_phase2_fastchg_current_ma_high;
		chg_err("current_limit = %d, charging_current = %d.\n",current_limit, charging_current);

		if ( chip->chg_ops && chip->chg_ops->input_current_write) {
		...
		if ( chip->chg_ops && chip->chg_ops->charging_current_write_fast) {
	...
	enum power_supply_property psp, const union power_supply_propval *val)
{
	...
	if (g_oplus_chip && g_oplus_chip->is_double_charger_support) {
		...
		case POWER_SUPPLY_PROP_EM_MODE:
			if (val->intval > 0) {
				g_oplus_chip->em_mode = true;
				g_oplus_chip->slave_charger_enable = true;
				battery_set_qc_current();
			} else {
				g_oplus_chip->em_mode = false;
			}
			break;
		default:
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_SUB_CURRENT:
		if (g_oplus_chip->charger_exist && g_oplus_chip->is_double_charger_support) {
			icharging = oplus_gauge_get_batt_current();
			if (icharging >= 0) {
				val->intval = 0;
			} else {
				iprimary = bq2589x_adc_read_charge_current();
				if ((icharging + iprimary) < 0) {
					val->intval = icharging + iprimary;
				} else {
					val->intval = 0;
				}
				chg_err("icharging=%d,iprimary=%d,isubchg=%d\n", icharging, iprimary,val->intval);
			}
		} else {
			val->intval = 0;
		}
...
EXPORT_SYMBOL(oplus_get_usbtemp_burn_volt);
