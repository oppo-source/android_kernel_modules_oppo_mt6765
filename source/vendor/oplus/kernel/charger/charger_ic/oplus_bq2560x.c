...
#define BQ_INPUT_CURRENT_COLD_TEMP_MA		2000
#define BQ_INPUT_CURRENT_NORMAL_TEMP_MA 	2000
#define BQ_INPUT_CURRENT_WARM_TEMP_MA		2000
#define BQ_INPUT_CURRENT_WARM_TEMP_HVDCP_MA	1200
#define BQ_INPUT_CURRENT_HOT_TEMP_MA		2000
...
static int bq2560x_enable_charger(struct bq2560x *bq)
{
	...
	if (IS_ERR_OR_NULL(bq->subchg_enable)) {
                pr_err("get subchg_enable fail\n");
                return -EINVAL;
	} else {
...
static int bq2560x_disable_charger(struct bq2560x *bq)
{
	...
	ret = bq2560x_update_bits(bq, BQ2560X_REG_01, REG01_CHG_CONFIG_MASK, val);
	if (IS_ERR_OR_NULL(bq->subchg_disable)) {
		pr_err("get subchg_disable fail\n");
		return -EINVAL;
	} else {
		pinctrl_select_state(bq->pinctrl, bq->subchg_disable);
	}
	pr_err("%s\n",__func__);
	return ret;
}
...
int oplus_bq2560x_set_current_without_aicl(int curr)
{
        chg_err("curr = %d", curr);
        return bq2560x_set_input_current_limit(g_bq, curr);
}
...
static irqreturn_t bq2560x_irq_handler(int irq, void *data)
{
	...
	bq->power_good = mt6357_get_vbus_status();
	bq->bq2560x_is_bc_en = bq->power_good;
	...
	if (prev_pg == bq->power_good && bq->chg_type != CHARGER_UNKNOWN
		&& NONSTANDARD_CHARGER != bq->chg_type) {
		pr_info("%s prev_pg & now_pg is same!\n", __func__);
		return IRQ_HANDLED;
	}
#endif
...
int irq_gpio_init(struct bq2560x *bq)
{
...
int oplus_bq2560x_is_charging_enabled(void)
{
	...
	ret = bq2560x_read_byte(g_bq, &val, BQ2560X_REG_01);

	if (!ret)
		g_bq->charge_enabled = !!(val & REG01_CHG_CONFIG_MASK);
...
int oplus_bq2560x_charger_suspend(void)
{
	...
	if (boot_mode == META_BOOT || g_oplus_chip->dischg_flag == true) {  //META mode or Burn-proof trigger
		bq2560x_enter_hiz_mode(g_bq);
	} else {
		oplus_bq2560x_set_aicr(100);
...
int oplus_bq2560x_charger_unsuspend(void)
{
	...
	if (boot_mode == META_BOOT || g_oplus_chip->dischg_flag == false) {  //META mode or Burn-proof trigger
...
//	void (*check_is_iindpm_mode) (void);
	...
	.input_current_write_without_aicl = oplus_bq2560x_set_current_without_aicl,
#endif
};
...
static int oplus_subchg_gpio_init(struct bq2560x *bq)
{
	if (!bq) {
		pr_err("chip not ready!\n");
		return -EINVAL;
	}

	bq->pinctrl = devm_pinctrl_get(bq->dev);
	if (IS_ERR_OR_NULL(bq->pinctrl)) {
		pr_err("get subch_en_pinctrl fail\n");
		return -EINVAL;
	} else {
		pr_err("get subch_en_pinctrl ok\n");
	}
