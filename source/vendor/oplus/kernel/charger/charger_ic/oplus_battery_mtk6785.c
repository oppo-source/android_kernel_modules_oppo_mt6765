...
int oplus_chg_get_battery_btb_temp(void)
{
	...
	if (!g_oplus_chip) {
		printk(KERN_ERR "[OPLUS_CHG][%s]: oppo_chip not ready!\n", __func__);
		return 25;
	}
...
void oplus_chg_choose_gauge_curve(void)
{
	if (!pinfo)
		set_charge_power_sel(CHARGER_NORMAL_CHG_CURVE);

	if (oplus_voocphy_get_adapter_type() == ADAPTER_SVOOC) {
		set_charge_power_sel(CHARGER_FASTCHG_SVOOC_CURVE);
	} else if (oplus_chg_get_charger_subtype() == CHARGER_SUBTYPE_QC
			|| oplus_chg_get_charger_subtype() == CHARGER_SUBTYPE_PD
			|| oplus_voocphy_get_adapter_type() == ADAPTER_VOOC20
			|| oplus_voocphy_get_adapter_type() == ADAPTER_VOOC30) {
		set_charge_power_sel(CHARGER_FASTCHG_VOOC_AND_QCPD_CURVE);
	} else {
		set_charge_power_sel(CHARGER_NORMAL_CHG_CURVE);
	}
}
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	oplus_chip->con_volt = con_volt_20315;
