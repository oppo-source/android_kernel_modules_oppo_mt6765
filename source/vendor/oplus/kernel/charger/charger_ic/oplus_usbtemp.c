...
#endif
        chg_err("[OPLUS_USBTEMP] timer is up now.\n");
...
static void oplus_usbtemp_set_recover(struct oplus_chg_chip *chip)
{
	if (alarmtimer_get_rtcdev()) {
		alarm_start_relative(&chip->usbtemp_alarm_timer, ms_to_ktime(usbtemp_recover_interval));
	} else {
...
void oplus_update_usbtemp_current_status(struct oplus_chg_chip *chip)
{
	...
	/*10W and 18W not process usb temp cool down.*/
