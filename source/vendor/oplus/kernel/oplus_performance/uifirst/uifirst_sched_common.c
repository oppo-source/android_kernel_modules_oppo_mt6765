...
#ifdef CONFIG_CAMERA_OPT
#ifdef CONFIG_OPLUS_FEATURE_AUDIO_OPT
	if (se_task->static_ux == 1 || (se_task->camera_opt && sysctl_camera_opt_enabled && !sysctl_sched_impt_tgid)) {
#else
...
{
	return (is_task_util_over(p, sysctl_boost_task_threshold) || sysctl_launcher_boost_enabled) && !is_ux_task_prefer_cpu_for_scene(p, cpu);
}
#ifdef CONFIG_OPLUS_SYSTEM_KERNEL_QCOM
...
void sched_assist_im_systrace_c(struct task_struct *tsk, int tst_type)
{
	...
	preempt_disable();
	event_trace_printk(mark_addr, "C|10001|short_run_%d_%d|%d\n", tsk->pid, tst_type, !!tsk->oplus_task_info.im_small);
	preempt_enable();
}
#endif
