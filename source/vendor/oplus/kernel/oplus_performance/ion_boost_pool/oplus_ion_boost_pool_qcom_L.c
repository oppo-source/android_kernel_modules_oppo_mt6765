...
/* static inline void boost_pool_clear_page(struct page *page, int order) */
/* { */
...
static int fill_boost_page_pool(struct ion_page_pool *pool)
{
	...
	/* boost_pool_clear_page(page, pool->order); */
		    ...
		    int order)
{
	...
	if (boost_pool_nr_pages(pool) < pool->low + (SZ_256M >> PAGE_SHIFT)) {
		/* boost_pool_clear_page(page, order); */
				     ...
				     size_t count, loff_t *ppos)
{
	...
	if (mutex_trylock(&boost_pool->prefill_mutex)) {
		...
		pr_info("%s: set high wm => %dMib. current avail => %dMib\n",
			current->comm, nr_pages >> 8, mem_avail >> 8);
...
static int set_tsk_scheduler(struct task_struct *tsk, bool fifo)
{
	...
	return sched_setscheduler_nocheck(tsk, policy, &param);
}
					 ...
					 char *name)
{
	...
	if (IS_ERR_OR_NULL(boost_pool->proc_info)) {
		...
		pr_info("procfs entry /proc/boost_pool/%s allocated. data=%p\n",
			name, boost_pool);
	}
