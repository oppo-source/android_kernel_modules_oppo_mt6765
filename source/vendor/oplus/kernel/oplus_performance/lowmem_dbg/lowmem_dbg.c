...
#ifdef CONFIG_QCOM_KGSL
extern int __weak read_each_kgsl_process_private(int (*callback)(const struct kgsl_process_private *kp,
					  void *private), void *private);
int __weak read_each_kgsl_process_private(int (*callback)(const struct kgsl_process_private *kp,
					  void *private), void *private)
{
	return 0;
}
...
static inline int dump_mem_detail(enum mem_type type, bool verbose)
{
	...
	case MEM_ION_USED:
		...
		break;
	case MEM_GPU:
#if defined(CONFIG_QCOM_KGSL) || IS_ENABLED(CONFIG_MTK_GPU_SUPPORT)
