...
struct fpc1020_data {
        ...
        struct pinctrl_state *pstate_irq_default;
...
int fpc_parse_pwr_list(struct fpc1020_data* fpc_dev) {
    ...
    for_each_available_child_of_node(np, child) {
        ...
        case FP_POWER_MODE_GPIO:
            ...
            pr_err("set fpc_pwr %u output %d \n", child_node_index, (pwr_list[child_node_index].poweron_level == 0 ? 1: 0));
            break;
...
static int fpc1020_irq_probe(struct platform_device *pldev)
{
        ...
        fpc1020->pstate_irq_default = NULL;
        fpc1020->dev = dev;
        dev_info(fpc1020->dev, "-->%s\n", __func__);
        ...
        if (g_use_gpio_power_enable == 1) {
            ...
            fpc1020->pstate_irq_default = pinctrl_lookup_state(fpc1020->pinctrl, "gpio_fpc_irq");
            if (IS_ERR(fpc1020->pstate_irq_default)) {
                dev_err(&pldev->dev, "Can't find gpio_fpc_irq pinctrl state\n");
                //return PTR_ERR(fpc1020->pstate_irq_default);
            } else {
                pinctrl_select_state(fpc1020->pinctrl, fpc1020->pstate_irq_default);
            }
        }
