...
#include "fpsensor_spi_tee_qsee.h"
...
static void fpsensor_power_off(void)
{
     FUNC_ENTRY();
	if(g_fpsensor->vddfp == NULL) {
	    fpsensor_debug(ERR_LOG,"g_fpsensor->vddfp is null.\n");
		return ;
	}
    /* power_enabled flag */
    if (g_fpsensor->power_enabled == 0) {
        regulator_disable(g_fpsensor->vddfp);
        g_fpsensor->power_enabled = 1;
		fpsensor_debug(ERR_LOG,"fpsensor power is off.\n");
    }
    FUNC_EXIT();
	return ;
}

static void fpsensor_power_on(void)
{
    FUNC_ENTRY();
	if(g_fpsensor->vddfp == NULL) {
	    fpsensor_debug(ERR_LOG,"g_fpsensor->vddfp is null.\n");
    ...
    /* power_enabled flag */
    if (g_fpsensor->power_enabled == 1) {
        regulator_enable(g_fpsensor->vddfp);
        g_fpsensor->power_enabled = 0;
	    fpsensor_debug(ERR_LOG,"fpsensor power is on.\n");
    }
	FUNC_EXIT();
	return ;
}
...
static int fpsensor_probe(struct platform_device *pdev)
{
	...
	if( FP_CHIPONE_C7332 != get_fpsensor_type()){
		pr_err("%s, found not fpsensor sensor\n", __func__);
		status = -EINVAL;
		return status;
	}
...
#if 1
    fpsensor_dev->vddfp = regulator_get(&fpsensor_dev->spi->dev, "vccfp");
    if (IS_ERR_OR_NULL(fpsensor_dev->vddfp)) {
        ret = PTR_ERR(fpsensor_dev->vddfp);
        fpsensor_debug(ERR_LOG,"get vddfp regulator failed,ret=%d", ret);
        //return ret;
    }else{
        printk("fpsensor maybe not find vccfp , it is not err and null\n");
		
        if (regulator_count_voltages(fpsensor_dev->vddfp) > 0) {
            printk("fpsensor: %s 684 ",__func__);
            ret = regulator_set_voltage(fpsensor_dev->vddfp, FP_VTG_MAX_UV,
                                    FP_VTG_MAX_UV);
            if (ret) {
                fpsensor_debug(ERR_LOG,"vddfp regulator set_vtg failed ret=%d", ret);
                regulator_put(fpsensor_dev->vddfp);
                return ret;
            }
			
			
            ret = regulator_enable(fpsensor_dev->vddfp);
            if (ret) {
                fpsensor_debug(ERR_LOG,"enable vddfp regulator failed,ret=%d", ret);
            }
