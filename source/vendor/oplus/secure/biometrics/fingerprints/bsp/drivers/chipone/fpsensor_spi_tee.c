  ...
  //  u32 ints[2] = {0, 0};
   ...
   // spidev_gpio_as_int(fpsensor);
	...
	if (node){
		...
		if (fpsensor->irq <= 0) {
			fpsensor_debug(ERR_LOG, "IRQ invalid, get irq from other node\n");
			node = of_find_compatible_node(NULL, NULL, "mediatek,fingerprint");
			if (node) {
				pdev = of_find_device_by_node(node);
				if (pdev == NULL) {
					fpsensor_debug(ERR_LOG, "can not find device by node\n");
					return -1;
				}
			} else {
				fpsensor_debug(ERR_LOG, "can not find compatible node\n");
				return -1;
			}

			/* get silead irq_gpio resource */
			fpsensor->irq_gpio = of_get_named_gpio(pdev->dev.of_node, "chipone,irq-gpio", 0);
			if (!gpio_is_valid(fpsensor->irq_gpio)) {
				fpsensor_debug(ERR_LOG, "IRQ GPIO invalid \n");
				return -1;
			}
...

{
    ...
    if(enable && onoff_cnt)
   {
      fpsensor_debug(ERR_LOG, "power on,onoff_cnt=%d\n",onoff_cnt);
        if (!IS_ERR_OR_NULL(g_fpsensor->fp_cs_func)) {
            pinctrl_select_state(g_fpsensor->pinctrl1, g_fpsensor->fp_cs_func);//set cs func
            fpsensor_debug(DEBUG_LOG, "%s, pinctrl_select_state fp_cs_func\n", __func__);
        }
    ...
    else if ((!enable)  && (!onoff_cnt))
   {
       fpsensor_debug(ERR_LOG, "power off,onoff_cnt=%d\n",onoff_cnt);
        if (!IS_ERR_OR_NULL(g_fpsensor->fp_cs_gpio)) {
            pinctrl_select_state(g_fpsensor->pinctrl1, g_fpsensor->fp_cs_gpio);//set cs func
            fpsensor_debug(DEBUG_LOG, "%s, pinctrl_select_state fp_cs_gpio\n", __func__);
        }
...
#endif
{
    ...
    if (CHIPONE_LDO_DISABLE == 1 ) {
        ...
        fpsensor_dev->fp_cs_gpio = pinctrl_lookup_state(fpsensor_dev->pinctrl1, "fpsensor_cs_gpio");
        if (IS_ERR(fpsensor_dev->fp_cs_gpio)) {
            ret = PTR_ERR(fpsensor_dev->fp_cs_gpio);
            fpsensor_debug(ERR_LOG, "fpsensor_dev Cannot find fp pinctrl fp_cs_gpio! ret = 0x%08x\n", ret);
        }
        fpsensor_dev->fp_cs_func = pinctrl_lookup_state(fpsensor_dev->pinctrl1, "fpsensor_cs_func");
        if (IS_ERR(fpsensor_dev->fp_cs_func)) {
            ret = PTR_ERR(fpsensor_dev->fp_cs_func);
            fpsensor_debug(ERR_LOG, "fpsensor_dev Cannot find fp pinctrl fp_cs_func! ret = 0x%08x\n", ret);
        }
    }
