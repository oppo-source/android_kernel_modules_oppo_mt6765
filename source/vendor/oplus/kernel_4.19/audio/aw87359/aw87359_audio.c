...
static char *aw87359_dspk_name = "aw87359_spk_music.bin";
static char *aw87359_drcv_name = "aw87359_drcv.bin";
//static char *aw87359_abspk_name = "aw87359_abspk.bin";
static char *aw87359_abspk_name = "aw87359_spk_voice.bin";
...
#if defined(CONFIG_SND_SOC_AW87359)
int aw87359_spk_low_voltage_status = 0;
void aw87359_audio_spk_low_voltage_status(int bStatus)
{
    aw87359_spk_low_voltage_status = bStatus;

    if(aw87359 == NULL) {
        pr_err("%s: aw87359 is NULL\n", __func__);
        return;
    }

    if (aw87359->hwen_flag == 1) {
        if (aw87359_spk_low_voltage_status > 0) {
            pr_info("%s: aw87359 write vol reg 0x04 when playing\n", __func__);
            aw87359_i2c_write(aw87359,REG_CPOVP, 0x04);//set as 7V
        } else {
            pr_info("%s: aw87359 write vol reg normal when playing\n", __func__);
            if(aw87359->dspk_cfg_update_flag == 0){
                aw87359_i2c_write(aw87359,REG_CPOVP, aw87359_dspk_cfg_default[10]);
            }
            if(aw87359->dspk_cfg_update_flag == 1){
                aw87359_i2c_write(aw87359,REG_CPOVP, aw87359_dspk_cnt->data[10]);
            }
        }
    }
    return;
...
#if defined(CONFIG_SND_SOC_AW87359)
    pr_err("%s flag=%d, vol_status=%d, reg_data[3]=0x%x\n",
        __func__, aw87359->dspk_cfg_update_flag, aw87359_spk_low_voltage_status, aw87359_dspk_cnt->data[3]);
    if (aw87359->dspk_cfg_update_flag == 1) {  /*update firmware data*/
        if(aw87359_spk_low_voltage_status > 0){
            for (i = 0; i < aw87359_dspk_cnt->len; i = i+2) {
                if(i == 2){
                    /*if the status of spk-low-vol is enable,
                    set the third reg as 0x04 directly(set as 7v)*/
                    aw87359_i2c_write(aw87359,
