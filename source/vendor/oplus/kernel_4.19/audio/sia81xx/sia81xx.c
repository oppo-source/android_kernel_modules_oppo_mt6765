	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_debug("[debug][%s] %s: ucontrol = %ld, rst = %d is_sia_chip = %d \r\n",
		LOG_FLAG, __func__, ucontrol->value.integer.value[0], sia81xx->rst_pin,is_sia_chip);
...
int sia81xx_volume_boost_power_get(
	...
	struct snd_ctl_elem_value *ucontrol)
{
	if ((sia81xx_audio_extern_config == 2) && (is_aw_chip != 1))
	{
