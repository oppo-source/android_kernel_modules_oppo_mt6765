			...
			if (cmd->request) {
				...
				if (hba->latency_hist_enabled &&(!blk_rq_is_passthrough(cmd->request))){
					if(req_op(cmd->request) == REQ_OP_WRITE || req_op(cmd->request) == REQ_OP_WRITE_SAME){
						io_update_latency_hist(&hba->io_lat_write, ktime_us_delta(lrbp->compl_time_stamp,lrbp->issue_time_stamp), blk_rq_sectors(cmd->request));
					}else if(req_op(cmd->request) == REQ_OP_READ){
						io_update_latency_hist(&hba->io_lat_read, ktime_us_delta(lrbp->compl_time_stamp,lrbp->issue_time_stamp), blk_rq_sectors(cmd->request));
					}else{
						io_update_latency_hist(&hba->io_lat_other, ktime_us_delta(lrbp->compl_time_stamp,lrbp->issue_time_stamp), blk_rq_sectors(cmd->request));
					}
				}
			}
#endif
